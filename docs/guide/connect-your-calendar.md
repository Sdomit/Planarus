...
with an Approvo project. You run every step yourself; nothing here happens
...
> Google or Microsoft).
> **Safety:** your login stays with Google/Microsoft — Approvo only receives an
...
- **Push** — events you create in Approvo's Calendar are copied to your Google/
  Microsoft calendar.
- **Pull** — events from that calendar appear in Approvo, alongside your
...
Approvo encrypts the calendar tokens it stores. Generate a key once, in
...
Copy the line it prints — that's your `AGENTBOARD_CALENDAR_ENC_KEY`. Keep it
...
## Step 3 — Configure Approvo

Add the values to your environment (e.g. an `.env` file that `apps/api` reads).
Set only the provider(s) you registered:

```
AGENTBOARD_CALENDAR_ENC_KEY=<the key from Step 1>

AGENTBOARD_CALENDAR_GOOGLE_CLIENT_ID=<Google client id>
AGENTBOARD_CALENDAR_GOOGLE_CLIENT_SECRET=<Google client secret>

AGENTBOARD_CALENDAR_MICROSOFT_CLIENT_ID=<Microsoft client id>
AGENTBOARD_CALENDAR_MICROSOFT_CLIENT_SECRET=<Microsoft client secret>
```
...
1. Open Approvo → pick a project → **Calendar** (left sidebar, under Workspace).
...
You entered your password on Google/Microsoft's page, never in Approvo.

## Step 5 — Sync

In the same **Sync** dialog, click **Sync now** on the connection. Approvo
...
- **Disconnect** (in the Sync dialog) deletes the stored token for that
  connection. Approvo can no longer reach that calendar.
- To revoke from the other side too, remove Approvo's access in your
  [Google Account permissions](https://myaccount.google.com/permissions) or
  Microsoft [My Apps](https://myapps.microsoft.com/).
- Removing `AGENTBOARD_CALENDAR_ENC_KEY` (or the client ids) from the
