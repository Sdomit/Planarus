...
``AGENTBOARD_EXTERNAL_API_ENABLED`` or makes AgentBoard reachable by ChatGPT; the
server URL is a placeholder, and AgentBoard never creates or manages a public
...
_AUTH_DESCRIPTION = (
    "AgentBoard API key sent as a Bearer token: "
    "`Authorization: Bearer agbk_<key_id>_<secret>`. The key is issued locally in "
    "AgentBoard, scoped to one workspace and project set, and shown only once. "
    "Never embed a real key in this document or reveal it in conversation."
)

_GENERAL_NOTE = (
    "Read-only and pending-proposal access to a single, local-first AgentBoard "
    ...
    "a local human approves and applies it in AgentBoard. AgentBoard is "
...
_SERVER_DESCRIPTION = (
    ...
    "loopback-bound AgentBoard external API. AgentBoard never creates or manages "
    "this host, tunnel, or reverse proxy."
)

_PROJECT_ID_DESC = (
    "AgentBoard project id, exactly as returned by a prior scoped read. Do not "
    "invent ids."
)
...
_PROBLEM_SPECS = {
    ...
    "PayloadTooLarge": ("Request body exceeds the AgentBoard size limit.", False),
    ...
    "ServiceUnavailable": ("AgentBoard data is temporarily unavailable.", False),
}
_STATUS_TO_REF = {
    401: "Unauthorized",
    403: "Forbidden",
...
def _proposal_result_schema() -> dict:
    return {
        ...
        "properties": {
            "approval_id": {
                "type": "string",
                "description": "Local AgentBoard approval id; check it with getApprovalStatus.",
            },
        ...
        "/docs/{doc_id}/excerpt": {
            "get": _operation(
                ...
                parameters=[
                    _path_param(
                        "doc_id",
                        "AgentBoard document id from a prior listDocs response. Do not invent ids.",
                    ),
                    _max_chars_param(),
                ],
            )
        ...
        "/approvals/{approval_id}/status": {
            "get": _operation(
                ...
                parameters=[
                    _path_param(
                        "approval_id",
                        "AgentBoard approval id returned when a proposal was created. Do not invent ids.",
                    )
                ],
            )
        },
    ...
    return {
        "/proposals/task": {
            "post": _operation(
                ...
                "Creates a pending approval in the local AgentBoard Approval Queue. No task "
        ...
        "/proposals/task/{task_id}": {
            "post": _operation(
                ...
                parameters=[
                    _path_param(
                        "task_id",
                        "AgentBoard task id from a prior listTasks response. Do not invent ids.",
                    )
                ],
...
def _info(title_suffix: str, profile_note: str) -> dict:
    return {
        "title": f"AgentBoard External API — GPT Actions ({title_suffix})",
