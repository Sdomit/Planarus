...
def _folder_root_label(project: Project) -> str:
    """#115: in auth (multi-user) mode the on-disk root is server-owned, so its
    absolute path is not written into the generated pack (which is served over
    the API). Local single-user mode keeps it — the one operator owns the box.
    Local output is unchanged, so golden tests stay byte-stable."""
    from app.core.config import settings

    if settings.auth_enabled:
        return "managed (server-owned)"
    return _or(project.folder_path, "unset")
...
def _project_body(ctx: RenderContext) -> list[str]:
    ...
    return [
        ...
        "",
        "## Key links",
        f"- Folder root: {_folder_root_label(p)}",
        "",
        "## Source",
