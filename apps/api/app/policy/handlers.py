...
def _check_status(session: Session, project_id: str, status: str, entity_type: str) -> None:
    """Phase 15.5: an applied status must be a built-in or one of the project's
    custom statuses (the policy layer couldn't check this — it has no project
    scope). Applies to task and decision writes."""
    if status not in status_option_service.allowed_status_keys(session, project_id, entity_type):
...
def _apply_task_create(session: Session, project_id: str, patch: dict) -> tuple[str, str]:
    _check_status(session, project_id, patch.get("status", "backlog"), "task")
    now = now_utc()
    task = Task(
        id=new_id("tsk"),
...
def _apply_task_update(session: Session, target_id: str, patch: dict) -> tuple[str, str]:
    ...
    if patch.get("status") is not None:
        _check_status(session, task.project_id, patch["status"], "task")
...
def _apply_decision_create(session: Session, project_id: str, patch: dict) -> tuple[str, str]:
    _check_status(session, project_id, patch.get("status", "proposed"), "decision")
    now = now_utc()
    decision = Decision(
        id=new_id("dec"),
        project_id=project_id,
