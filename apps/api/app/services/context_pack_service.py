...
def _truncate_at_heading(text: str, max_chars: int) -> tuple[str, bool]:
    ...
    kept.append("(truncated — open the document in AgentBoard for the full text)")
    return "\n".join(kept), True
...
def _constraints_lines(governance: list[_Governance]) -> tuple[list[str], list[str]]:
    ...
    warnings: list[str] = []
    lines = [
        "Scope is defined by AgentBoard governance files (checksum-verified,"
        " authoritative):",
    ]
...
def _assemble_body(
    ...
    out += _section(
        ...
        [
            "- This pack is local. It leaves AgentBoard only when you copy it into"
            " an external tool.",
